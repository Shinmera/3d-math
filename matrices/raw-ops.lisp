...
(define-template smatop <op> <st> <s> <t> (x m s)
  (let ((type (type-instance 'mat-type <s> <t>))
    ...
    `((declare (type ,(lisp-type type) x m)
      ...
      (let ((ma ,(place-form type 'arr 'm))
            (xa ,(place-form type 'arr 'x))
            (s (,(upgraded-type <t> <st>) s)))
        (do-times (i 0 ,(attribute type :len 'm) 1 x)
          (setf (aref xa i) (,<t> (,<op> (aref ma i) s))))))))
...
(define-template meuler <s> <t> (x m)
  (unless (member <s> '(3 4)) (template-unfulfillable))
  (unless (member <t> '(f32 f64)) (template-unfulfillable))
  (let ((type (type-instance 'mat-type <s> <t>))
    ...
    (flet ((i (y x)
      ...
      `((declare (type ,(lisp-type type) m)
        ...
        (let ((ma ,(place-form type 'arr 'm))
              (va ,(place-form vtype 'arr 'x)))
          (cond ((= -1 ,(i 3 1))
                 (let* ((phi (,<t> 0))
                        (theta (,<t> (/ PI 2)))
                        (psi (+ phi (atan ,(i 1 2) ,(i 1 3)))))
                   (setf (aref va 0) psi (aref va 1) theta (aref va 2) phi)))
                ((= +1 ,(i 3 1))
                 (let* ((phi (,<t> 0))
                        (theta (,<t> (/ PI -2)))
                        (psi (+ (- phi) (atan (- ,(i 1 2)) (- ,(i 1 3))))))
                   (setf (aref va 0) psi (aref va 1) theta (aref va 2) phi)))
                (T
                 (let* ((theta1 (- (the ,<t> (asin ,(i 3 1)))))
                        (theta2 (- (,<t> PI) theta1))
                        (ctheta1 (cos theta1))
                        (ctheta2 (cos theta2))
                        (psi1 (atan (/ ,(i 3 2) ctheta1) (/ ,(i 3 3) ctheta1)))
                        (psi2 (atan (/ ,(i 3 2) ctheta2) (/ ,(i 3 3) ctheta2)))
                        (phi1 (atan (/ ,(i 2 1) ctheta1) (/ ,(i 1 1) ctheta1)))
                        (phi2 (atan (/ ,(i 2 1) ctheta2) (/ ,(i 1 1) ctheta2))))
                   ;; Pick whichever solution has the smaller absolute
                   (if (< (+ (abs theta1) (abs psi1) (abs phi1))
                          (+ (abs theta2) (abs psi2) (abs phi2)))
                       (setf (aref va 0) psi1 (aref va 1) theta1 (aref va 2) phi1)
                       (setf (aref va 0) psi2 (aref va 1) theta2 (aref va 2) phi2)))))
          x)))))
...
(do-type-combinations mat-type define-meuler)
