...
(define-template qtowards <t> (x from to)
  (let ((type (type-instance 'quat-type <t>))
    ...
    `((declare (type ,(lisp-type type) x from to)
               (return-type ,(lisp-type type))
               inline)
      (let ((ff (,(lisp-type vtype) from))
            (tt (,(lisp-type vtype) to)))
...
(define-template set-qangle <t> (angle x)
  (let ((type (type-instance 'quat-type <t>)))
    `((declare (type ,(lisp-type type) q)
               ...
               (return-type ,<t>)
               inline)
      (setf ,(place-form type 3 'q) (cos (* (,<t> 0.5) angle)))
      angle)))
  ...
  (let ((type (type-instance 'quat-type <t>))
    ...
    `((declare (type ,(lisp-type type) x)
      ...
      (let* ((f (vunit* dir))
        ...
        (!vc u f r)
        (,(compose-name #\/ 'qtowards <t>) x +vz+ f)
        (,(compose-name #\/ 'q*v <t>) object-up x +vy+)
...
(do-quat-combinations define-qfrom-angle)
(do-quat-combinations define-qtowards)
(do-quat-combinations define-qangle)
(do-quat-combinations define-set-qangle)
(do-quat-combinations define-2quatop (+ - * / min max))
(do-quat-combinations define-squatop (+ - * / min max) (<t> real))
(do-quat-combinations define-1quatop (- / abs identity))
(do-quat-combinations define-2quatreduce (and) (= ~= < <= >= >) boolean)
(do-quat-combinations define-squatreduce (and) (= ~= < <= >= >) (<t> real) boolean)
(do-quat-combinations define-2quatreduce (or) (/=) boolean)
(do-quat-combinations define-squatreduce (or) (/=) (<t> real) boolean)
(do-quat-combinations define-2quatreduce (*) (+) (<t>)) ; dot
(do-quat-combinations define-1quatreduce (+) (sqr) <t>) ; sqrlen
(do-quat-combinations define-q*q)
(do-quat-combinations define-q*v)
(do-quat-combinations define-qexpt)
(do-quat-combinations define-qlookat)
(do-quat-combinations define-qmat (3 4))
(do-quat-combinations define-qfrom-mat (3 4))
