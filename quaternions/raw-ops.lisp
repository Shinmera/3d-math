...
(define-template dual-q*q <t> (x a b)
  (let ((type (type-instance 'dual-quat-type <t>)))
    `((declare (type ,(lisp-type type) x a b)
               (return-type ,(lisp-type type)))
      (let* ((a (qunit a))
             (b (qunit b))
             (r*d (q* (qreal a) (qdual b)))
             (d*r (q* (qdual a) (qreal b))))
        (declare (dynamic-extent a b r*d d*r))
        (!q* (qreal x) (qreal a) (qreal b))
        (!q+ (qdual x) r*d d*r))
      x)))
